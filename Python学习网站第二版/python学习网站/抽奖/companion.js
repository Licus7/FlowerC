...
getAvailablePets() {
    try {
        ...
        // 标准化数据格式
        const availablePets = myPets.map(pet => {
            // 确保有正确的图片URL
            let image = pet.image || this.getPokemonImage(pet.id);
            
            // 如果image是错误地址，替换为有效地址
            if (image.includes('dicebear.com') || !image.includes('pokemon')) {
                image = this.getPokemonImage(pet.id);
            }
        ...
        console.log('可用精灵:', availablePets);
        return availablePets;
        
    } catch (error) {
        console.error('获取可用精灵失败:', error);
        return [];
    }
}
...
function initializeSelectionPage() {
    ...
    debugBtn.onclick = function() {
        console.log('=== 调试信息 ===');
        console.log('myPets:', JSON.parse(localStorage.getItem('myPets') || '[]'));
        console.log('currentCompanion:', JSON.parse(localStorage.getItem('currentCompanion') || '{}'));
        console.log('companionSettings:', JSON.parse(localStorage.getItem('companionSettings') || '{}'));
        alert('调试信息已输出到控制台');
    };
...
function loadAvailablePets() {
    ...
    availablePets.forEach(pet => {
        ...
        // companion.js - 修改 loadAvailablePets 函数中的卡片生成
petCard.innerHTML = `
...
// 选择精灵函数
// companion.js - 修改 selectCompanion 函数
