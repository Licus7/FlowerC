# FlowerC网站项目 - 需求规格说明书（第二轮）

## 修订历史

| 版本 | 日期 | 作者 | 描述 |
| :--- | :--- | :--- | :--- |
| 1.0 | 2025年9月 | FlowerC团队 | 初始版本，定义核心学习流程与最基础功能。 |
...
**第一版目标**：已验证核心学习路径的可行性，实现了基于10章内容的教学、练习、Boss挑战及基础用户系统。
...
Boss战低保真模型图：
<img src="boss战设计.jpg" width=90% alt="boss设计">
**3.3.1 章节练习（规则更新）**
- 题型、次数限制不变。
- **FR-EX-01 练习结算规则**：
    - 提交后，系统计算得分（百分比）。
...
### 7.1 新增功能验收清单
- [x] **FR-UC-01/02/03**：个人中心页面能正确显示进度总览、金币总额、成就徽章、精灵收藏数及陪伴精灵。
- [x] **FR-LE-02**：完整浏览章节内容后，顶部金币数增加50，并有提示。
- [x] **FR-EX-01**：章节练习得分≥60%时，获得100金币；得满分时，共获得150金币。提示清晰。
- [x] **FR-PK-02**：点击抽奖按钮，消耗100金币，播放动画后显示结果。抽到重复精灵自动转为30金币。
- [x] **FR-PK-04**：在个人中心可以切换不同的已拥有精灵作为陪伴精灵。

### 7.2 数据持久化验收清单
- [x] 所有金币变动（学习、练习、登录、抽奖）均被准确记录，刷新页面后余额不变。
- [x] 获得的精灵被永久记录，可在个人中心查看。
- [x] 用户指定的“陪伴精灵”状态被保存。
