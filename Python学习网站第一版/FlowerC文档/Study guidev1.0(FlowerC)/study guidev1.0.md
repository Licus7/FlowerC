...
B - 软件项目的复杂程度和大小（软件规模包括功能数量、代码复杂度、团队规模等多方面因素）
...
class TightlyCoupled:
    def process(self):
        ...
        # 直接调用具体实现
        other_module.specific_implementation()
```
...
class ProgressTracker:
    ...
    def update_progress(self, chapter, score):
        """只负责进度跟踪相关功能"""
        self.progress_data[chapter] = score
    
    def get_completion_rate(self):
        """计算完成率"""
        return len(self.progress_data) / TOTAL_CHAPTERS

# 低内聚的反例
class MiscManager:
    def handle_user(self): ...    # 用户管理
    def process_learning(self): ...  # 学习处理
    def manage_game(self): ...    # 游戏管理
```
